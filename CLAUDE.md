...
Claude Code 스킬(플러그인) 모음 레포지토리. 13개 플러그인으로 구성되며, 런타임 빌드 시스템 없이 셸 스크립트와 SKILL.md 기반으로 동작한다.
...
- **sub-frontend-engineer** — React/Vite 프론트엔드 (컴포넌트 설계, 상태 관리, 빌드 최적화)
- **sub-codebase-explorer** — 다언어 코드베이스 분석 (의존성 그래프, Git hotspot, 데드코드, MSA API/이벤트 토폴로지 → 호출별/토픽별 개별 md 생성, ARCHITECTURE/ONBOARDING 자동 작성)
...
# sub-frontend-engineer: 프론트엔드 스택 감지
plugins/sub-frontend-engineer/skills/sub-frontend-engineer/scripts/detect-frontend-stack.sh [프로젝트경로]

# sub-codebase-explorer: 다언어 스택 + MSA 시그널 감지
plugins/sub-codebase-explorer/skills/sub-codebase-explorer/scripts/discover-stack.sh [프로젝트경로]
# sub-codebase-explorer: 모듈 의존성 그래프 + 순환 탐지
plugins/sub-codebase-explorer/skills/sub-codebase-explorer/scripts/extract-dependencies.sh [프로젝트경로]
plugins/sub-codebase-explorer/skills/sub-codebase-explorer/scripts/detect-circular-deps.sh [프로젝트경로]
# sub-codebase-explorer: Git hotspot 분석
plugins/sub-codebase-explorer/skills/sub-codebase-explorer/scripts/analyze-git-hotspot.sh [프로젝트경로] ["6 months ago"]
# sub-codebase-explorer: MSA API 호출/이벤트 추출 + 개별 md 일괄 생성
plugins/sub-codebase-explorer/skills/sub-codebase-explorer/scripts/extract-msa-api-calls.sh [MSA프로젝트경로]
plugins/sub-codebase-explorer/skills/sub-codebase-explorer/scripts/extract-msa-events.sh [MSA프로젝트경로]
plugins/sub-codebase-explorer/skills/sub-codebase-explorer/scripts/generate-msa-docs.sh [MSA프로젝트경로] [출력디렉토리]
...
- sub-frontend-engineer: Discover → Design → Implement → Verify
- sub-codebase-explorer: Discover → Map(+MSA) → Diagnose → Document
